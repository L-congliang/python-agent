...
def main():
    ...
    for config in configs:
        ...
        for metric in metrics_keys:
            values = [
                ...
                for r in all_rounds
                if not r["anomaly"] and config in r["data"]
            ]
    ...
    for metric in metrics_keys:
        print(f"\n--- {metric} ---", flush=True)
        print(f"{'config':<20} {'mean':>8} {'min':>8} {'max':>8} {'range':>8} {'std':>8}", flush=True)
        for config in configs:
            s = aggregate[config][metric]
            fmt = ".0%" if metric in ("correct_rate", "memory_hit_rate") else ".1f"
            print(f"{config:<20} {s['mean']:>8{fmt}} {s['min']:>8{fmt}} {s['max']:>8{fmt}} {s['range']:>8{fmt}} {s['std']:>8{fmt}}", flush=True)
    ...
    output_path = Path("docs/test-reports/formal-experiment-v2-raw.json")
