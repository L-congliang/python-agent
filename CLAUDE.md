...
| 2 | 多轮对话记忆管理 | 对话历史怎么压缩、怎么召回、上下文窗口怎么控制 |
| 3 | 多 Agent 路由 | 多个 agent 时，谁来处理什么任务，怎么分发 |
| 4 | 监控与评测 | agent 的表现怎么量化、怎么发现退化、怎么端到端评测 |
| 5 | 工具调用兜底 | 工具出错怎么办、检索不全怎么补、模型幻觉怎么兜底 |
...
`feature_list.json` 与 master-roadmap 对齐，按 Phase 组织（P0-P6）。
