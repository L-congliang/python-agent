...
1. **新增 6 个 V4-A 高难度任务** — 全部 L3/L4，使用更严格的 verifier
2. **V4 任务集统计** — L3/L4 占比 66.7%，forbidden_reads 8 个，allowed_files+no_extra_changes 6 个
3. **Smoke Test** — 6/6 任务全部通过
4. **正式实验** — memory_on vs memory_off 各 5 轮

### V4 实验结果

| Config | 轮次 | Clean | correct_rate |
|--------|------|-------|--------------|
| memory_on | 5 | 5 | **96.67%** |
| memory_off | 5 | 4 | **100.00%** |

### 关键结论

**V4 高难度任务仍然没有让记忆系统效果显现。**

- memory_off 反而表现更好 (100% vs 96.67%)
- 模型本身能力太强，能够通过工具调用弥补记忆缺失
- 当前任务设计可能还不够极端，无法体现记忆价值

### 下一步建议

1. 设计更极端的任务（需要跨 5+ 轮、跨 5+ 文件）
2. 考虑在能力较弱的模型上测试
3. 或者重新评估记忆系统的目标和价值
