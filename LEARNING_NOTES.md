...
| 工具 | 用途 | 类比 |
|------|------|------|
| **Write** | 创建新文件、整体覆盖 | 新建文档、替换整个文件 |
| **Edit** | 局部精确替换 | Word 的"查找替换" |

**为什么分开？**

- Write 操作简单，直接写入整个内容
- Edit 需要先读取文件，找到匹配位置，精确替换
- 职责清晰，符合单一职责原则
- 对齐 Claude Code 的设计
...
file_edit(
    file_path="main.py",
    old_string='print("old message")',
    new_string='print("new message")'
)
...
**为什么用 old_string 而不是行号？**

- 行号会变：Edit 后文件行数可能改变，后续操作行号失效
- old_string 语义明确：直接表达"要替换什么"
- 容错性好：即使文件其他部分变了，只要 old_string 还在就能替换

### 唯一匹配约束

**问题**：如果 old_string 在文件中出现多次，替换哪一个？
...
# Edit(old_string="print('hello')", new_string="print('world')")
# 替换哪一个？不确定！
```

**解决方案**：严格要求 old_string 只能出现一次，否则报错。

```python
# 报错：old_string 匹配到 2 处，要求唯一匹配
# 请提供更具体的 old_string，或使用 replace_all=True
```

**replace_all=True**：显式 opt-in 批量替换，用户明确知道会替换所有匹配。

### 自动创建目录

**问题**：写入 `src/utils/helper.py` 时，如果 `src/utils/` 不存在怎么办？
...
# 选项 B：自动创建（推荐）
os.makedirs(parent_dir, exist_ok=True)  # 递归创建目录
```

**为什么推荐自动创建？**

- Agent 创建新文件时，目录结构往往也是新的
- 让 Agent 专注于"写什么"，而不是"先创建目录"
- `exist_ok=True` 避免目录已存在时的报错

### 缓存一致性

**问题**：Write/Edit 修改文件后，Read 工具的缓存会过期。

```python
# 1. Read("main.py") → 缓存内容 + mtime=1000
# 2. Write("main.py", "new content") → mtime 变成 1001
# 3. Read("main.py") → 缓存 mtime=1000 ≠ 磁盘 mtime=1001 → 重新读取
```

**解决方案**：Write/Edit 执行后主动更新缓存。

```python
def _update_cache(file_path: str, content: str, ctx: ToolUseContext) -> None:
    mtime = os.path.getmtime(file_path)
    ctx.file_cache.update(file_path, content, mtime)
```

**为什么不用"删除缓存"？**

- 删除缓存 → 下次 Read 要重新读磁盘
- 更新缓存 → 下次 Read 直接用缓存（更快）
- Write/Edit 刚写入的内容，Read 大概率马上会读（Agent 验证写入结果）

### 权限检查

**为什么要权限确认？**

- Write 会覆盖已有文件（数据丢失风险）
- Edit 会修改已有文件（可能改错）
- 用户需要知道 Agent 在改什么文件
...
```python
def execute_file_write(input: dict, ctx: ToolUseContext) -> ToolResult:
    file_path = input["file_path"]

    # 1. 检查权限（执行前）
    if not ctx.permissions.can_write(file_path):
        return ToolResult(output="Permission denied", is_error=True)
...
def _check_disk_space(path: str, content_size: int) -> bool:
    usage = shutil.disk_usage(os.path.dirname(path))
    return usage.free > content_size
```

**为什么不用 `os.statvfs`？**

- `os.statvfs` 是 Unix-only，Windows 不支持
- `shutil.disk_usage` 跨平台，Windows/Linux/Mac 都能用

### Windows 兼容性

**问题**：`os.statvfs` 在 Windows 上不存在。

```python
# ❌ Unix-only
import os
stat = os.statvfs("/")
print(stat.f_bavail * stat.f_frsize)  # 可用空间

# ✅ 跨平台
import shutil
usage = shutil.disk_usage("/")
print(usage.free)  # 可用空间
```

**mypy 类型检查**：即使用 try/except 捕获 `AttributeError`，mypy 仍会报错。需要加 `# type: ignore[attr-defined]`。

### mypy --strict 类型问题

**问题**：`dict.get()` 返回 `Any | None`，不能当 `str` 用。

```python
# ❌ mypy 报错
def execute(input: dict) -> ToolResult:
    file_path = input.get("file_path")  # Any | None
    content = input.get("content")      # Any | None
    return ToolResult(output=file_path + content)  # 类型不安全

# ✅ 正确写法
def execute(input: dict) -> ToolResult:
    file_path: str = input["file_path"]  # 明确类型
    content: str = input["content"]      # 明确类型
    return ToolResult(output=file_path + content)
```

**为什么用 `input["key"]: str = ...` 而不是 `input.get("key")`？**

- `input["key"]` 会在 key 不存在时抛 KeyError（fail-fast）
- `input.get("key")` 返回 None，后续代码可能报更奇怪的错
- 类型注解 `: str` 让 mypy 知道这是 str，不是 Any

### Q&A 回顾：设计时问过的问题

**Q1: Write 和 Edit 有什么区别？**

- **Write**：创建新文件或整体覆盖。输入是完整的文件内容。
- **Edit**：局部替换。输入是 old_string + new_string，只改文件的一部分。

类比：Write 是"重写整篇文档"，Edit 是"查找替换"。

**Q2: old_string 是什么？**

文件中要被替换的文本片段。必须精确匹配（包括空格、换行）。
...
**Q3: 为什么 old_string 必须唯一匹配？**

防止误替换。如果 old_string 出现多次，Edit 不知道替换哪一个，所以报错让用户明确意图。

**Q4: replace_all 是什么？**

批量替换所有匹配项。需要显式设置 `replace_all=True`，因为批量替换风险更高。

```python
# 文件中有 3 个 "old_func"
# Edit(old_string="old_func", new_string="new_func", replace_all=True)
# 结果：3 个都变成 "new_func"
```

**Q5: 为什么要自动创建目录？**

Agent 创建新文件时，目录结构往往也是新的。如果每次都要先创建目录，Agent 需要额外调用 Bash 工具，浪费一轮交互。

**Q6: 缓存更新有什么用？**

Write/Edit 修改文件后，Read 工具的缓存会过期。主动更新缓存，下次 Read 直接用新内容，不需要重新读磁盘。

**Q7: 为什么 Write/Edit 需要权限确认？**
